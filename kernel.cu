...
__global__ void __launch_bounds__(blocksize, 1) nonceGrindc(uint32_t *const __restrict__ headerIn, uint32_t *const __restrict__ midstateIn, uint32_t *const __restrict__ nonceOut)
{
	static const uint32_t k[64] =
	{
		0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5,
	...
	const uint32_t id = headerIn[16] + (blockDim.x * blockIdx.x + threadIdx.x) * npt;
...
// Only used for determining hashrate
...
void nonceGrindcuda(cudaStream_t cudastream, uint32_t threads, uint32_t *blockHeader, uint32_t *midState, uint32_t *nonceOut)
{
	...
	nonceGrindc << <grid, blocksize, 0, cudastream >> >(blockHeader, midState, nonceOut);
	e = cudaGetLastError();
	if(e != cudaSuccess)
	{
		printf("Error: %s\n", cudaGetErrorString(e));
	}
	totalhashes += grid * blocksize * npt;
}
	...
	*nonceOut = 0;
	...
	for(i = 0; i < 60; i++)
	{
		...
		if(*nonceOut != 0)
		{
			...
			*nonceOut = 0;
			totalNonces++;
			remainingHeader[16] += grid * blocksize * npt;
			break;
		}
		remainingHeader[16] += grid * blocksize * npt;
	}
